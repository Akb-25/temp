    ...
    if issubclass(type(net),nn.Sequential) or issubclass(type(net),nn.ModuleList):
        for submoduleID, layer in net.named_children():
            ...
            if type(net.get_submodule(submoduleID)) in PBG.modulesToReplace:
                setattr(net,submoduleID,replacePredefinedModules(net.get_submodule(submoduleID),  getPretrainedPBVar(submoduleID)))
...
class PAIModulePyThread(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        for outIndex in range(0,len(self.layerArray)):
            ...
            # print("The index in this is: ", outIndex)
            if(len(self.layerArray) > 1):
                for inIndex in range(0,outIndex):
                    # print("Current Out shape is here: ", currentOut.shape)
                    # print("Skip weight shape is here: ", self.skipWeights[outIndex][inIndex, :].shape)
                    # print("pbOuts weight shape is here: ", pbOuts[inIndex].shape)
