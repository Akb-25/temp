...
def test(model, device, test_loader, optimizer, scheduler, args):
    ...
    if(restructured): 
        ...
        optimArgs = {'params':model.parameters(), 'lr':0.1, 'momentum':0.9,'nesterov':True}
        # schedArgs = {'step_size':1, 'gamma': args.gamma}
        schedArgs = {
        'milestones': [91, 136],
        'gamma': 0.1
...
def main():
    ...
    parser.add_argument('--save-name', type=str, default='r6PB')
    ...
    model = model.to(device)
    print("Model being used after converting is: ", model)
    ...
    PBG.pbTracker.setOptimizer(optim.SGD)
    PBG.pbTracker.setScheduler(MultiStepLR)
    optimArgs = {'params':model.parameters(), 'lr':0.1, 'momentum':0.9, 'nesterov':True}
    # schedArgs = {'step_size':1, 'gamma': args.gamma}
    schedArgs = {
    'milestones': [91, 136],
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # torch.save(model.state_dict(), f"New/{epoch}_model.pt")
    ...
    torch.save(model.state_dict(), "r6pai.pt")
