...
log_file = open("pruningInit.csv", mode="w", newline='')
log_file_2 = open("pruningInitBest.csv", mode="w", newline='')
...
def prune(rec, dec, classifier, train_loader, optimizer, criterion):
    ...
    while True:
        ...
        torch.save(rec, f"modelsInit/{pruning_iteration}_rec.pth")
        torch.save(dec, f"modelsInit/{pruning_iteration}_dec.pth")
        torch.save(classifier, f"modelsInit/{pruning_iteration}_classifier.pth")
        torch.save(model, f"modelsInit/{pruning_iteration}_model.pth")
...
if __name__ == '__main__':
    ...
    # torch.save(model, "start_model.pth")
    # sys.exit()
    # rec = model.rec
    # dec = model.dec
