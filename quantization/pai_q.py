...
if __name__ == '__main__':
    ...
    print(model)
    # sys.exit()
    torch.save(model, "Q/PAI.pt")
    torch.save(model.state_dict(), "compressedOriginal")
    ...
    test_loss, test_acc, test_auc = utils.evaluate_classifier(
        model.rec, test_loader, args=args, classifier=model.classifier, reconst=True, num_sample=1,
    ...
    torch.save({'weights': q_weights, 'scales': scales, 'zero_points': zero_points}, "Q/compressed_model.pt")
    print("Compressed model weights are saved")
    
    with open("original_raw.pkl", "wb") as f:
    ...
    with open("compressed_raw.pkl", "wb") as f:
        q_numpy = {k: v.cpu().numpy() for k, v in q_weights.items()}
        pickle.dump(q_numpy, f, protocol=pickle.HIGHEST_PROTOCOL)

    orig_size = os.path.getsize("original_raw.pkl")
    quant_size = os.path.getsize("compressed_raw.pkl")
...
#     orig_size = os.path.getsize("compressedOriginal")
#     quant_size = os.path.getsize("compressedWeights.pt")
